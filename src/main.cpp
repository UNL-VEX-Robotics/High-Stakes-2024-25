...
motor Left_Motor1 = motor(PORT6, ratio6_1, true);
...
motor Right_Motor1 = motor(PORT5, ratio6_1);
motor Right_Motor2 = motor(PORT3, ratio6_1);
...
led Rusharm= led(Brain.ThreeWirePort.H);
...
void auton(void){
    ...
    task([]() -> int {
        waitUntil(Inertial.heading(deg) > 135);
        Rusharm.set(!Rusharm);
        return 0;
    });
    Drivetrain.turnTo(90);
    //Get Rings From +,- Corner
    Drivetrain.turnTo(180);
    Drivetrain.driveFor(8,1.5);
    wait(0.5,seconds);
    Drivetrain.arcFor(left,1,-80);
    // //Puts Mogo in +,- Corner
    ...
    //Mogo Rush B
    //Intake_group.spin(forward,75,percent);
    //Drivetrain.driveFor(40);
    //Drivetrain.swingFor(right,45);
   // Drivetrain.driveFor(24);
    //Rushclaw.set(!Rushclaw);
}
...
void usercontrol(void) {
    ...
    while (true) {
        ...
        if (Controller.ButtonUp.pressing() && !upWasPressing) runIntake = !runIntake;        
        if (Controller.ButtonL2.pressing()) Intake_group.spin(reverse, 100, percent);
        else if (runIntake) Intake_group.spin(forward, 100, percent);
